...
# -----------------------------------------
#TABIX=os.getenv("TABIX")
#SAMTOOLS=os.getenv("SAMTOOLS")
cgitb.enable(format="text",logdir="/tmp/python/")
# -----------------------------------------
DEFAULT_LINE_LEN = 60
DATA_DIR="."
...
def defaultHeader (desc) :
    try:
        desc["IMGT_type"]=desc["IMGT_type"]
    except:
        desc["IMGT_type"]=""
    hdr = ">%s:%s:%s:%s" % (desc["genome"], "|".join(desc["ID"].split()), "|".join(desc["IMGT_type"].split()), "+".join(desc["regions"].split()))
...
def getSequenceFromFaidx (desc) :
    ...
    try:
        gpath = desc["genomeUrl"].strip("/").split("/")[-1]
        try:
            track = desc["track"]
        except:
            track = "assembly"
        regions = desc["regions"].split(":")[0]
        regions2 = desc["regions"].split(":",2)[1]
        try:
            fulllength = int(desc["totalLength"])
        except:
            fulllength = int(desc["length"])
        path = "%s/%s/%s/%s.txt" % (DATA_DIR, gpath, track, regions)
        pos=regions2.replace("..","-").split("-")
        with open(path,"r") as file:
            file.seek(int(pos[0])-1) #Because starts at +1
            r = file.read(fulllength)
    except Exception as ex:
        error("Unable to access data: {0}".format(str(e.args[0])).encode("utf-8"))
        exit(1)
    return r
...
def validateSequenceOptions (opts) :
    if not opts.descriptors:
        error("No descriptors on sequences.")
    ndescs = 0
    tLength = 0
